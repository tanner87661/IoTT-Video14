...
const uint32_t lnPingIntervall = 300000; //300 Seconds to send ping string to MQTT server on Topic lnPing
...
const uint32_t bushbyRefreshIntv = 1800000; //check every 30 Minutes
...
bool connectToWifi()
{
  ...
  if (Serial.available())
  {
    wifiManager.startConfigPortal("BlueBox", "IoTTCloud");
  }
...
//        Serial.println(msg);
        ...
        else
        {
          ...
          {
            String newCmd = doc["Cmd"];
            if (newCmd == "Switch")
            {
              if (doc.containsKey("CmdParams"))
              {
                JsonArray cmdParams = doc["CmdParams"];
                for (int i=0;i<cmdParams.size();i++)
                {
                  String newParam = cmdParams[i];
                  if (newParam == "SendSwitch")
                    if (doc.containsKey("JSONData"))
                    {
                ...
                for (int i=0;i<cmdParams.size(); i++)
                {
                  ...
                  if (newParam == "DisplayData")
                    if (doc.containsKey("JSONData"))
                    {
                      ...
                      handleDisplayJSONRequests(doc["JSONData"]);
                      //wsRequest |= 0x0008;
                    }
                  if (newParam == "DisplayTest")
                    if (doc.containsKey("JSONData"))
                    {
                      ...
                      handleDisplayTestJSONRequests(doc["JSONData"]);
                      //wsRequest |= 0x0008;
                    }
                  if (newParam == "Emulator")
                    if (doc.containsKey("JSONData"))
                    {
                      ...
                      handleEmulatorJSONRequests(doc["JSONData"]);
                      //wsRequest |= 0x0008;
                    }
                }
              }
                ...
                for (int i=0;i<cmdParams.size(); i++)
                {
                  ...
                  if (newParam == "CfgData")
                    if (doc.containsKey("JSONData"))
                    {
                      ...
                      handleConfigJSONRequests(doc["JSONData"]);
                      wsRequest |= 0x0001;
                    }
                  if (newParam == "SignalData")
                    if (doc.containsKey("JSONData"))
                    {
                      ...
                      handleSignalJSONRequests(doc["JSONData"]);
                      wsRequest |= 0x0002;
                    }
                }
              }
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  { 
    if (useInputMode == 2) //MQTT
    {
      switch ((byte)doc["Data"][0])
      {
        case 0xB0: if (useBushby && isBushby) break; //OPC_SW_REQ
        case 0xB1:; //OPC_SW_REP
        case 0xBD:  //OPC_SW_ACK
        {
          uint16_t swiAddr = (((byte)doc["Data"][1] & 0x7F)) + (((byte)doc["Data"][2] & 0x0F)<<7);
          Serial.print("Swi Change ");
          Serial.println(swiAddr + 1);
          uint16_t byteNr = trunc(swiAddr/4);
          uint8_t inpPosStat = ((byte)doc["Data"][2] & 0x30)>>4; //Direction and ON Status
          inpPosStat |=0x01;
...
//          Serial.println(swiPos[byteNr]);
        ...
        {
//          ledcWriteTone(buzPin, buzFreq);
          uint16_t inpAddr = (((byte)doc["Data"][1] & 0x7F)<<1) + (((byte)doc["Data"][2] & 0x0F)<<8) + (((byte)doc["Data"][2] & 0x20)>>5);
...
//          Serial.print("->");
          uint8_t inpStatus = ((byte)doc["Data"][2] & 0x10)>>4;
          if (inpStatus > 0)
...
//          Serial.println(bdStatus[byteNr]);
        ...
        {
          if (((byte)doc["Data"][1] == 0x0B) && ((byte)doc["Data"][2] == 0x7F))
          {
            byte recData[6];
            for (int i=0; i < 5; i++)
            {
              recData[i] = (byte)doc["Data"][i+4];
              if (i > 0)
...
//          Serial.println("Receive SL_RD");
          if (((byte)doc["Data"][1] == 0x0E) && ((byte)doc["Data"][2] == 0x7F)) //Status slot with OpSw
          {
            isBushby = (((byte)doc["Data"][6] & 0x04) > 0);
    ...
    if (dataFile)
    {
      ...
      if (!error)
      {
        if (doc.containsKey("useWifi"))
          useWifi = bool(doc["useWifi"]);
        if (doc.containsKey("NetBIOSName"))
          strcpy(netBIOSName, doc["NetBIOSName"]);
...
//          strcpy(staPassword, root["staPassword"]);
        if (doc.containsKey("useDHCP"))
          useDHCP = bool(doc["useDHCP"]);
        if (doc.containsKey("staticIP"))
          strcpy(staticIP, doc["staticIP"]);
        if (doc.containsKey("staticGateway"))
          strcpy(staticGateway, doc["staticGateway"]);
        if (doc.containsKey("staticNetmask"))
          strcpy(staticNetmask, doc["staticNetmask"]);
        if (doc.containsKey("staticDNS"))
          strcpy(staticDNS, doc["staticDNS"]);
        if (doc.containsKey("useAP"))
          useAP = bool(doc["useAP"]);
        if (doc.containsKey("apName"))
          strcpy(apName, doc["apName"]);
        if (doc.containsKey("apGateway"))
          strcpy(apGateway, doc["apGateway"]);
//        if (doc.containsKey("apUser"))
 //         strcpy(apUser, doc["apUser"]);
        if (doc.containsKey("apPassword"))
          strcpy(apPassword, doc["apPassword"]);
        if (doc.containsKey("useNTP"))
          useNTP = bool(doc["useNTP"]);
        if (doc.containsKey("NTPTimeServer"))
        {
//          String hlpStr = doc["NTPTimeServer"];
          strcpy(ntpServer, doc["NTPTimeServer"]); // = hlpStr.c_str();
          NTPch.setNTPServer(ntpServer);
        }
        if (doc.containsKey("ntpTimeZone"))
          timeZone = int(doc["ntpTimeZone"]);
        if (doc.containsKey("useBushby"))
          useBushby = bool(doc["useBushby"]);
        if (doc.containsKey("useInputMode"))
          useInputMode = doc["useInputMode"];
        ...
        if (doc.containsKey("lnBCTopic"))
        {
          strcpy(lnBCTopic, doc["lnBCTopic"]);
        }
        if (doc.containsKey("lnEchoTopic"))
        {
          strcpy(lnEchoTopic, doc["lnEchoTopic"]);
        }
      }
    }
  } 
  ...
  if (SPIFFS.exists("/signals.cfg"))
  {
    ...
    if (dataFile)
    {
      ...
      if (!error)
      {
        ...
        {
          globalBrightness = doc["Brightness"]["PercentVal"];
        }
        if (doc.containsKey("Fading"))
        {
          fadeCtrlSteps = doc["Fading"]["PercentVal"];
        }

        Serial.println(String(ESP.getFreeHeap()));
        if (doc.containsKey("SigHeads"))
        {
          JsonArray sigHeads = doc["SigHeads"];
...
String handleConfigJSON()
{
  ...
  //config data stuff
  doc["useWifi"] = int(useWifi);
  doc["NetBIOSName"] = netBIOSName;
  doc["useDHCP"] = int(useDHCP);
  doc["staticIP"] = staticIP;
  doc["staticGateway"] = staticGateway;
  doc["staticNetmask"] = staticNetmask;
  doc["staticDNS"] = staticDNS;
  ...
  doc["useNTP"] = int(useNTP);
  doc["NTPServer"] = ntpServer;
  doc["ntpTimeZone"] = timeZone;
  doc["useBushby"] = int(useBushby);
  doc["useInputMode"] = useInputMode;

  JsonObject data = doc.createNestedObject("mqttServer");
  data["ip"] = mqtt_server;
  data["port"] = mqtt_port;
  data["user"] = mqtt_user;
  data["password"] = mqtt_password;

  doc["lnBCTopic"] = lnBCTopic;
  doc["lnEchoTopic"] = lnEchoTopic;
...
String handleRuntimeJSON()
{
  ...
  DynamicJsonDocument doc(1500);
    ...
    doc["BytesTransmitted"] = bytesTransmitted;
    doc["LNLoadBps"] = networkLoad.average();
    doc["LNLoad100"] = 100 * networkLoad.average() / (lnLoadIntervall / 0.6);
  serializeJson(doc, response);
  Serial.println(response);
  return response;
}
